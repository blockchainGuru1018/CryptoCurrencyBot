...
class AbstractSpreadToCSV(UpdateAction):
    ...
    def __init__(self, filename: str, should_override: bool, spread_threshold: Optional[int] = None) -> None:
        ...
        self.tri_header = ['exchange', 'spread', 'time_pretty',
                           'currency_pair', 'timestamp']
        self.filename = filename
        self.should_override = should_override

    def run_inter(self, spreads: List[SpreadDetection], exchanges: List[Exchange], timestamp: float) -> None:
    ...
    def run_tri(self, spreads: List[TriSpreadDetector], exchanges: List[Exchange], timestamp: float) -> None:
        # Create file if it does not yet exist
        if not os.path.isfile(self.filename):
            f = open(self.filename, 'w+')
